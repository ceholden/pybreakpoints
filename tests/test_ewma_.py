...
from pybreakpoints import ewma_
...
def test__sd_moving_range(x, k, sd):
    result = ewma_._sd_moving_range(x, k)
...
def test__c4():
    ...
    expected = np.array([0.7978845608, 0.9775593519, 0.9881702533,
                         ...
                         0.9972565726])
    for n_, expected_ in zip(n, expected):
        result = ewma_._c4(n_)
        np.testing.assert_allclose(result, expected_)


def test__sd_sample(x):
    expected = 0.3415928
    result = ewma_._sd_sample(x)
...
def test__ewma_smooth(x):
    ...
    result = ewma_._ewma_smooth(x, x.mean(axis=0), lambda_=0.2)
...
def test__ewma_boundary(x):
    ...
    stddev = ewma_._sd_moving_range(x, k=2)
    result = ewma_._ewma_boundary(x, stddev,
...
def test_EWMA(x):
    # NumPy
    result = ewma_.ewma(x, crit=2.7, lambda_=0.2, std_type='SD')
    assert (result.score - 33.67529) < 1e-5
    assert result.signif is False

    # Pandas and xarray
    for dt in (pd.Series, xr.DataArray):
        result = ewma_.ewma(dt(x), crit=2.7, lambda_=0.2, std_type='MR')
        assert result.signif is False

    # Test MAD
    result = ewma_.ewma(x, crit=2.7, lambda_=0.2, std_type='MAD')
    assert result.signif is False
